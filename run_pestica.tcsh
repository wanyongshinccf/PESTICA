...
if ( $physiofile != "" ) then
  	...
  	if ( $fastpmucorflag == 1 ) then # afni RETROICOR
    	...
    	matlab -nosplash -r "disp('Starting script...'); addpath $MATLAB_PESTICA_DIR; addpath $MATLAB_AFNI_DIR; addpath $MATLAB_EEGLAB_DIR; load RetroTS.PMU.mat; [RESP CARD] = retroicor_pmu('epi_00+orig','epi_base_mask+orig',SN, CARD, RESP,'rm.polort.xmat.1D'); exit;" 
  	endif
