function [fext, fcard,fresp]=readSiemesnPhysio_VENX(fname,TR,samplerate,downsamplerate)
% this function read PMU data (e.g. **.ext, *.resp, *.card) 
% created by Wanyong Shin, CCF 20230627
...
% external trigger
[ext extpeak tp_start_ext tp_end_ext] = readpmu_VENX(fname_ext);
...
tdim=length(xtrigs);
disp([num2str(tdim) ' of external trigger signals are detected. '])

% find time stamp
ms_dur_ext = tp_end_ext - tp_start_ext; % [ms]

% resp
[resp card_peak tp_start_resp tp_end_resp] = readpmu_VENX(fname_resp);
ms_dur_resp = tp_end_resp - tp_start_resp; % [ms]

% card
[card card_peak tp_start_card tp_end_card] = readpmu_VENX(fname_card);
