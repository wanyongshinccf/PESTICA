...
buckettemplate = [retroicordir '/retroicor_pmu.bucket+orig.HEAD'];
...
OptOut.verbose = 0;
if OPTIONS.RVHR
  OptOut.Prefix = [ep2d_filename(1:strfind(ep2d_filename,'+')-1) '.retroicor_pmu.RVHR'];    
else
...
OptOut.verbose = 0;
if OPTIONS.RVHR
  OptOut.Prefix = [ep2d_filename(1:strfind(ep2d_filename,'+')-1) '.retroicor_pmu.RVHR.bucket'];
else
...
if pmuQAflag
  ...
  % load average phase function of 28 subjects
  load([retroicordir '/PhaseFunc_3T_28.mat'])
  ...
  if isfield(RESP, 'phz_slc_reg')
    ...
    % test of averaged resp phase function based on the mean of 28 subjects
    ccval = corr(mean_rt_raw',RESP_avg.hpf');
    studt = ccval*sqrt(tdim-2)/sqrt(1-ccval^2);
    p = 1-tcdf(abs(studt),(tdim-2));
    if p > 0.05
        disp('average of resp phase function does not look typical')
        disp('check the sanity of respiratory physio signal')
    end
    
    % caluclate polarity corrected phase function (Eqn[7] in cPESTICA paper)
    if ccval < 0; mean_rt_raw = -1*mean_rt_raw; end
  ...
  if isfield(CARD, 'phz_slc_reg') 
    ...
    % Fig2 of a paper (Shin, Koenig & Lowe, 2022) can be generated here 
    ...
    % test of averaged resp phase function based on the mean of 28 subjects
    ccval = corr(mean_ct_raw',CARD_avg.hpf');
    studt = ccval*sqrt(tdim-2)/sqrt(1-ccval^2);
    p = 1-tcdf(abs(studt),(tdim-2));
    if p > 0.05
        disp('average of card phase function does not look typical')
        disp('check the sanity of cardiac physio signal')
    end
    
    % caluclate polarity corrected phase function (Eqn[7] in cPESTICA paper)
    if ccval < 0; mean_ct_raw = -1*mean_ct_raw; end
...
if cphaseshift
  % store coefficients in physio voxels
    cb1map = squeeze(bmap(:,:,:,1+4)); % card sine term
    ca1map = squeeze(bmap(:,:,:,2+4)); % resp cosine term
    
    % Table 2 from cPESTICA paper
    cPhzShft = atan2(ca1map,cb1map); % -> -pi to +pi

    % warp between -90 deg < < 90 deg 
    cPhzShft(find(cPhzShft<-pi/2)) = cPhzShft(find(cPhzShft<-pi/2))+pi;
    cPhzShft(find(cPhzShft>+pi/2)) = cPhzShft(find(cPhzShft>+pi/2))-pi;
    
    % bound between peak_pic1_w1 +/- pi/2
    cPhzShft(find(cPhzShft>CARD.hpf_phaSft_mean+pi/2)) ... 
           = cPhzShft(find(cPhzShft>CARD.hpf_phaSft_mean+pi/2))-pi;
    cPhzShft(find(cPhzShft<CARD.hpf_phaSft_mean-pi/2)) ... 
           = cPhzShft(find(cPhzShft<CARD.hpf_phaSft_mean-pi/2))+pi;
         
    % masking and subtract the mean
    cPhzShft = cPhzShft.*mask - CARD.hpf_phaSft_mean;
    
    % bucket
    bucket = zeros(xdim,ydim,zdim,2);
    bucket(:,:,:,1) = cPhzShft;
    bucket(:,:,:,2) = Fmap(:,:,:,3);
    % stat 
    binfo.BRICK_LABS='CardPhzShft~Card_Fstst';
    binfo.BRICK_STATAUX = [1 4 2 2*M DOF];
         
    % define option
    OptOut.Prefix = [ep2d_filename(1:strfind(ep2d_filename,'+')-1) '.retroicor_pmu.Cphz.bucket'];    
    [err,ErrMessage,InfoOut]=WriteBrik(bucket,binfo,OptOut);
end
