function [Opt, R, E] = RetroTS_CCF_PESTICA(SN)
% modified RetroTS AFNI matlab library (W.S)
% resp signal is not Fouier Series expanded but directly used as a regressor
% See Shin, Koenig and Lowe, Neuroimage, 2022
%
...
for (icol=1:1:length(R)),
    for (isl=1:1:Opt.Nslices),
        ...
        for (i=1:1:length(R(icol).tst)),
            ...
            R(icol).slc_reg(i,isl) = R(icol).v(imin);
        end
    end
end
...
%          end
%       end
% new direct respiratory regressor (W.S)
      for (i=1:1:Opt.Nslices),
         cnt = cnt + 1;
         Opt.RemlOut(:,cnt) = R.slc_reg(:,i);
         label = sprintf('%s s%d.Resp0 ;', label, i-1); % (W.S) debugged 20224114
      end
   end
   %Card
   if (Opt.Card_out),
   ...
   for (i=1:1:Opt.Nslices),
      ...
      if (Opt.Resp_out),
         ...
         cnt = cnt + 1;
         Opt.RemlOut(:,cnt) = R.slc_reg(:,i);
         label = sprintf('%s s%d.Resp0 ;', label, i-1); % (W.S) debugged
      end
      if (Opt.Card_out),
         %Card
         for (j=1:1:size(E.phz_slc_reg,2)),
